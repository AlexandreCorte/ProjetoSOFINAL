...
int rwlock_initialize(pthread_rwlock_t* lock){
    if (pthread_rwlock_init(lock, NULL)!=0){
        perror("rwlock_initialize_error");
        return -1;
    }
    return 0;
}
...
int rwlock_write_lock(pthread_rwlock_t* lock){
    if (pthread_rwlock_wrlock(lock)!=0){
        perror("rwlock_write_lock_error");
        return -1;
    }
    return 0;
}
...
void state_init() {
    ...
    for (size_t i = 0; i < INODE_TABLE_SIZE; i++) {
        rwlock_initialize(&inode_table[i].lock);
        freeinode_ts[i] = FREE;
    }
...
/*criar tabela com tamanho de INODE_TABLE_SIZE? -> mutex?*/
int inode_create(inode_type n_type) {
    mutex_lock(&inode_create_lock);
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                if (b == -1) {
                    ...
                    mutex_unlock(&inode_create_lock);
                    return -1;
                }
...
int inode_delete(int inumber) {
    ...
    mutex_lock(&inode_create_lock);
    rwlock_write_lock(&inode_table[inumber].lock);
    freeinode_ts[inumber] = FREE;
    ...
    if (inode->i_size > 0) {
        ...
        for (int i = 0; i <= blocks_to_free; i++) {
            ...
            if (data_block_free(inode->i_data_block[i]) == -1) {
                rwlock_unlock(&inode_table[inumber].lock);
                mutex_unlock(&inode_create_lock);
                return -1;
            }
    ...
    /* TODO: handle non-empty directories (either return error, or
...
/*
pthread_mutex_t trinco[MAX_OPEN_FILES];
*/

int add_to_open_file_table(int inumber, size_t offset, int flags) {
    mutex_lock(&open_table_lock);
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            open_file_table[i].flags = flags;
            mutex_unlock(&open_table_lock);
            return i;
        }
    }
    mutex_unlock(&open_table_lock);
...
open_file_entry_t *get_open_file_entry(int fhandle) {
    ...
    mutex_lock(&open_table_lock);
    open_file_entry_t * open_file = &open_file_table[fhandle];
    mutex_unlock(&open_table_lock);
    return open_file;
}
